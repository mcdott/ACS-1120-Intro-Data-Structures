# Generate a single, weighted (based on word frequency) random word 
# from a dictionary (created from a text file)
# import random
# import sys
...
# Generate a single, weighted (based on word frequency) random word 
# from a list of tuples (created from a text file)
import random
import sys
...
def weighted_random_word(filename):
    ...
    # Convert lines to a list of tuples (word, frequency)
    histogram = [(line.split()[0], int(line.split()[1])) for line in lines]

    # Calculate the sum of all frequencies
    total = sum(freq for _, freq in histogram)
