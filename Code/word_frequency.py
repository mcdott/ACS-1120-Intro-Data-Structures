...
# Or, using a list of lists instead of a dictionary:
import string

def histogram(source_text):
    """
    This function takes a string as input, and processes the string to return a histogram as a list of lists.
    """
    # Remove punctuation and convert to lowercase
    translator = str.maketrans('', '', string.punctuation)
    source_text = source_text.translate(translator).lower()
...
def unique_words(hist):
    """
    This function takes a histogram (list of word counts) and returns the number of unique words.
    """
    return len(hist)

def frequency(word, hist):
    """
    This function takes a word and a histogram (list of word counts), and returns the frequency of that word.
    """
    for sub_list in hist:
        if word == sub_list[0]:
            return sub_list[1]
    return 0
...
def main():
    ...
    # Compute some statistics
    frequencies = [sub_list[1] for sub_list in hist]
    mean = sum(frequencies) / len(frequencies)
    median = sorted(frequencies)[len(frequencies) // 2]
    mode = max(set(frequencies), key=frequencies.count)
